...
//! ```
//!
//! If you need more than a single `Alignment` of bytes you can create a vector of them
...
//! `align_first` returns an empty Vec with a capacity of at least `capacity` bytes.
...
//! ```
//!
//! `align_first_boxed`, `align_first_boxed_default`, and `align_first_boxed_cloned` all return a `Box<[T]>` with the
//! first element aligned to the at least `Alignment` bytes.
//! ```
//! use maligned::*;
